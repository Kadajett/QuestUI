...
function runShadcn(command: "init" | "add", args: string[]): void {
...
function registryBase(value: string | undefined): URL {
  if (!value) {
...
function forwardOption(args: string[], index: number, forwarded: string[]): number {
  ...
  if (arg === "--cwd=" || arg === "--path=") fail(`${arg.slice(0, -1)} requires a non-empty value.`)
  return index
}

function addName(names: string[], name: string): void {
  if (!components.includes(name)) fail(`Unknown component '${name}'. Run 'quest-ui list' for available names.`)
...
function parseFlagArgs(flagArgs: string[], names: string[], forwarded: string[]): string | undefined {
...
function parseAddArgs(args: string[]): AddOptions {
  ...
  if (separator !== -1) {
    for (const name of args.slice(separator + 1)) addName(names, name)
  }
  if (!names.length) fail("add requires at least one component name. Run 'quest-ui list' for available names.")
...
switch (command) {
  ...
  case "help":
    ...
    break
  case "list":
    if (args.some(arg => arg !== "--help" && arg !== "-h")) fail("list takes no arguments.")
