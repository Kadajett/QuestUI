...
import { planInit, type InitPlan } from "./init-project.js"
...
function registry(value: string | undefined): string {
  if (!value) throw new Error("Pass --registry <http(s)-base-url> or set QUEST_UI_REGISTRY.")
  const url = new URL(value)
  if (!/^https?:\/\//i.test(value) || !["http:", "https:"].includes(url.protocol) || !url.hostname || url.username || url.password || url.search || url.hash) throw new Error("Registry must be an absolute HTTP(S) base URL without credentials, query, or fragment.")
  url.pathname = `${url.pathname.replace(/\/+$/, "")}/`
  return new URL("quest-theme.json", url).href
}

function installFailure(child: SpawnSyncReturns<Buffer>): string | undefined {
  if (child.error) return child.error.message
  if (child.signal) return child.signal
  if (child.status !== 0) return String(child.status)
  return undefined
}
...
function install(packages: string[], development: boolean, cwd: string): void {
...
function writePlanFile(plan: InitPlan, file: InitPlan["files"][number]): void {
  const path = resolve(plan.cwd, file.path)
  const current = existsSync(path) ? readFileSync(path, "utf8") : undefined
  if (current !== file.before) throw new Error(`${file.path} changed after preflight; refusing to overwrite it.`)
  writeFileSync(path, file.after, { flag: file.before === undefined ? "wx" : "w" })
}

function verifyThemeFiles(cwd: string): void {
...
function rollback(cwd: string, written: readonly InitPlan["files"][number][]): void {
  for (const file of [...written].reverse()) {
    const path = resolve(cwd, file.path)
    if (!existsSync(path) || readFileSync(path, "utf8") !== file.after) continue
    if (file.before === undefined) unlinkSync(path)
    else writeFileSync(path, file.before)
  }
}

function execute(plan: InitPlan, themeUrl: string): void {
  install(plan.runtime, false, plan.cwd)
  install(plan.development, true, plan.cwd)
  const written: InitPlan["files"] = []
  try {
...
function printPlan(plan: InitPlan, themeUrl: string): void {
  console.log(`QuestUI Vite setup: ${plan.cwd}`)
  for (const file of plan.files) console.log(`${file.before === undefined ? "Create" : "Update"} ${file.path}`)
  if (plan.runtime.length) console.log(`Install runtime: ${plan.runtime.join(" ")}`)
  if (plan.development.length) console.log(`Install development: ${plan.development.join(" ")}`)
