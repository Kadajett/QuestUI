...
type Options = { name: string | undefined; cwd: string; output: string; overwrite: boolean }
...
function parse(args: string[]): Options {
  ...
  if (positionals.length > 1) fail("swizzle accepts one component at a time.")
  return { name: positionals[0], output: values.output, cwd: resolve(values.cwd), overwrite: values.overwrite }
}

export function swizzle(args: string[]): void {
  ...
  if (!options.name) fail("swizzle requires a component name. Run 'quest-ui swizzle --help'.")
  const item = componentCatalog.find(component => component.name === options.name || component.native === options.name)
  if (!item) fail(`Unknown QuestUI component '${options.name}'. Run 'quest-ui list'.`)
  const forwarded = ["swizzle", item.native, "--package", "@astryxdesign/core", "--output", options.output]
  if (options.overwrite) forwarded.push("--overwrite")
  console.log("Ejected source retains Astryx npm imports and requires StyleX compilation.")
  if (!runUpstream(`@astryxdesign/cli@${astryxVersion}`, "astryx", forwarded, options.cwd)) return
