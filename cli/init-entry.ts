...
import { apply, imports, namedImport, source, unusedName, type Change } from "./init-syntax.js"

function isCreateRootCall(node: ts.Node, createRoot: string): node is ts.CallExpression {
  return ts.isCallExpression(node) && ts.isIdentifier(node.expression) && node.expression.text === createRoot
}

function rootNames(file: ts.SourceFile, createRoot: string): Set<string> {
  const roots = new Set<string>()
  for (const statement of file.statements) {
    if (!ts.isVariableStatement(statement)) continue
    for (const declaration of statement.declarationList.declarations) {
      if (!ts.isIdentifier(declaration.name) || !declaration.initializer) continue
      if (isCreateRootCall(declaration.initializer, createRoot)) roots.add(declaration.name.text)
    }
  }
  return roots
}

function renderCall(statement: ts.Statement, roots: ReadonlySet<string>, createRoot: string): ts.CallExpression | undefined {
  if (!ts.isExpressionStatement(statement) || !ts.isCallExpression(statement.expression)) return undefined
  const call = statement.expression
  if (!ts.isPropertyAccessExpression(call.expression) || call.expression.name.text !== "render") return undefined
  const receiver = call.expression.expression
  if (isCreateRootCall(receiver, createRoot)) return call
  return ts.isIdentifier(receiver) && roots.has(receiver.text) ? call : undefined
}

function renderArgument(file: ts.SourceFile): ts.Expression {
  const createRoot = namedImport(file, "react-dom/client", "createRoot")
  if (!createRoot) throw new Error("src/main.tsx must import createRoot from react-dom/client.")
...
function isThemeWrapper(argument: ts.Expression, provider: string, theme: string | undefined): boolean {
  if (!theme || !ts.isJsxElement(argument) || argument.openingElement.tagName.getText() !== provider) return false
  return argument.openingElement.attributes.properties.some(attribute =>
...
function missingStyleImports(file: ts.SourceFile): string[] {
  const additions: string[] = []
  for (const path of ["@astryxdesign/core/reset.css", "@astryxdesign/core/astryx.css", "@/components/quest/fonts.css"]) {
    if (!imports(file, path).length) additions.push(`import ${JSON.stringify(path)}`)
  }
  return additions
}

function themeSetup(file: ts.SourceFile, argument: ts.Expression, theme: string | undefined, wrapped: boolean): {
  additions: string[]
  changes: Change[]
} {
  if (wrapped) return {additions: [], changes: []}
...
export function setupEntry(text: string): string {
  ...
  if (additions.length) changes.push({ start: 0, end: 0, text: `${additions.join("\n")}\n` })
